...
def assemble_global(nodes, elems, k=1.0):
    ...
    for e in range(nelems):
        conn = elems[e] #elem array
        coords = nodes[conn, :2]  # take x,y only from every node in elem
        Ke = element_stiffness_triangle(coords, k=k)
        for i_local, i_global in enumerate(conn):
            for j_local, j_global in enumerate(conn):
                rows.append(i_global)#paste in rows the value i_global
                cols.append(j_global)
                data.append(Ke[i_local, j_local])

    # build sparse matrix (COO(sparse) -> CSR(usefull to solve later)) 
    K = coo_matrix((data, (rows, cols)), shape=(nnodes, nnodes)).tocsr()# with .tocsr() change coo->csr
...
def apply_heat_flux(f, nodes, elems, heat_flux_bcs):
    ...
    for elem_id, edge_id, q in heat_flux_bcs:
        conn = elems[elem_id]
        coords = nodes[conn, :2]
        edge_nodes = { 1: [0,1], 2: [1,2], 3: [2,0]}[edge_id]
        n1, n2 = edge_nodes
        x1, y1 = coords[n1]
        x2, y2 = coords[n2]
...
def apply_convection(K, f, nodes, elems, conv_bcs):
    ...
    for elem_id, edge_id, h, Tinf in conv_bcs:
        ...
        # Contribution to K (2x2)
        Ke_edge = (h * L / 6) * np.array([[2,1],
                                          [1,2]])
        # Contribution to f (2x1)
        fe_edge = (h * L / 2) * np.array([1.0, 1.0]) * Tinf
        
        # Προσθήκη στα σωστά DOF
        for i_local, i_global in enumerate([n1,n2]):
            f[conn[i_global]] += fe_edge[i_local]
            for j_local, j_global in enumerate([n1,n2]):
                K[conn[i_global], conn[j_global]] += Ke_edge[i_local, j_local]
